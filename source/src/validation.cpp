...
#if defined(NDEBUG)
#error "AMMOcoin cannot be compiled without assertions."
#endif
...
{
    util::ThreadRename("ammocoin-scriptch");
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // AMMOcoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nAmmocoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nAmmocoinBadBlockBits)) {
            // accept AMMOcoin block minted with incorrect proof of work threshold
            return true;
        }
