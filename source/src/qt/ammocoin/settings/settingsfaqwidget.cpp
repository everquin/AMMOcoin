...
#include "qt/ammocoin/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(AMMOCOINGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableAMMO,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableAMMO,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableAMMO,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("AMMOcoin is a form of digital online money using blockchain technology "
    ...
    QString unspendableAMMOContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received AMMOcoin requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your AMMOcoin wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableAMMO->setText(unspendableAMMOContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of AMMOcoin with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("ammocoin-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::AMMO)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in AMMOcoin Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the AMMOcoin network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableAMMO, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableAMMO);});
    ...
    if (parent)
        connect(parent, &AMMOCOINGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
