...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("AMMOcoinMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("AMMOcoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("AMMOcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ammocoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AMMOcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("AMMOcoinMiner exception");
    } catch (...) {
        LogPrintf("AMMOcoinMiner exception");
    }

    LogPrintf("AMMOcoinMiner exiting\n");
}
