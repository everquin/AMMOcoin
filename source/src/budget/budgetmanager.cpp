...
uint256 CBudgetManager::SubmitFinalBudget()
{
    ...
    if (!mapUnconfirmedFeeTx.count(budgetHash)) {
        ...
        return UINT256_ZERO;
#else
        // Wallet disabled - cannot create collateral transaction
        LogPrint(BCLog::MNBUDGET,"%s: Wallet functionality disabled, cannot create collateral transaction\n", __func__);
        return UINT256_ZERO;
#endif
    }
...
bool CBudgetManager::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const int nHeight, bool fProofOfStake) const
{
    ...
    // Starting from AMMOcoin v6.0 masternode and budgets are paid in the coinbase tx of PoS blocks
