...
**Note:** The VM uses `ammocoin-source` as the directory name (when the repository was cloned). The same codebase in the repository is located in `source/`. Users may clone the repository with any directory name.
