...
"""Test ammocoind with different proxy configuration.

Test plan:
- Start ammocoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on ammocoind side:
...
- Create ammocoinds that connect to them
- Manipulate the ammocoinds using addnode (onetry) an observe effects
...
class ProxyTest(AMMOcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: ammocoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
