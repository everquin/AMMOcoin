...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/AMMOcoinParams"
    else
...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/ammocoin/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
AMMOcoin - install-params.sh

This script will install the AMMOcoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common AMMOcoin zkSNARK parameters. Note that it is
