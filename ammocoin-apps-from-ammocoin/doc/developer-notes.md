...
- [Development guidelines](#development-guidelines)
    - [General AMMOcoin Core](#general-ammocoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
AMMOcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
AMMOcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ammocoin-project/ammocoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ammocoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ammocoin --log_level=test_suite
$ valgrind -v --leak-check=full src/ammocoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ammocoin.coverage/index.html`.
```

**Sanitizers**
AMMOcoin can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as AMMOcoin Core, where everyone uses
...
pay attention to for reviewers of AMMOcoin Core code.

General AMMOcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: AMMOcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against AMMOcoin Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/AMMOcoin-Project/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with AMMOcoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:AMMOcoin-Project/AMMOcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ammocoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ammocoin-cli` and the GUI debug console use this table to determine how to
