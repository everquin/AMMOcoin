...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturn information about the given AMMOcoin address.\n"
                "Some of the information will only be present if the address is in the active wallet.\n"
                "Metadata for shield addresses is available only if the wallet is unlocked.\n"
                "{Result:\n"
                "  \"address\" : \"address\",              (string) The AMMOcoin address validated.\n"
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new AMMOcoin address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"ammocoinaddress\"    (string) The new ammocoin address\n"
...
UniValue getnewexchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new AMMOcoin exchange address for receiving transparent funds only.\n"
            ...
            "\nResult:\n"
            "\"ammocoinaddress\"    (string) The new ammocoin exchange address\n"
...
UniValue getnewstakingaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new AMMOcoin cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(dest) || isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AMMOcoin address");

    const std::string strLabel = (request.params.size() > 1 ? request.params[1].get_str() : "");

    const CKeyID* keyID = boost::get<CKeyID>(&dest);
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from AMMOcoin address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->HasAddressBook(*keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get AMMOcoin address from addressBook");
...
UniValue listdelegators(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) AMMOcoin address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new AMMOcoin address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"       (string, required) The ammocoin address to be associated with a label.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The ammocoin address to send to.\n"
...
static UniValue CreateColdStakeDelegation(CWallet* const pwallet, const UniValue& params, CTransactionRef& txNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AMMOcoin staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStakingAddress)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AMMOcoin spending address");
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"staking_addr\"      (string, required) The ammocoin staking address to delegate.\n"
            "2. \"amount\"            (numeric, required) The amount in PIV to delegate for staking. eg 100\n"
            "3. \"owner_addr\"        (string, optional) The ammocoin address corresponding to the key that will be able to spend the stake.\n"
...
UniValue viewshieldtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                ...
                "      \"address\" : \"ammocoinaddress\",     (string) The AMMOcoin address involved in the transaction\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"ammocoinaddress\",     (string) The ammocoin address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The ammocoin address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns the total amount received by the given ammocoinaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"address\"    (string, required) The ammocoin address for transactions.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield AMMOcoin addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The ammocoin address (either transparent or shield) is the key,\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a AMMOcoin address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of ammocoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) ammocoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"        (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"ammocoinaddress\"  (string) A ammocoin address associated with the keys.\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"ammocoinaddress\",    (string) The ammocoin address of the transaction.\n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"address\":\"ammocoinaddress\",    (string) The ammocoin address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            HelpExampleCli("signmessage", "\"ammocoinaddress\" \"test message\"") +
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 6)
        throw std::runtime_error(
                ...
                "1. minconf          (numeric, optional, default=1) The minimum confirmations to filter\n"
                "2. maxconf          (numeric, optional, default=9999999) The maximum confirmations to filter\n"
                "3. \"addresses\"    (string) A json array of ammocoin addresses to filter\n"
                "    [\n"
                "      \"address\"   (string) ammocoin address\n"
                "      ,...\n"
                "    ]\n"
                ...
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",  (string) the ammocoin address\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid AMMOcoin address: ") + input.get_str());
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "   {\n"
            "     \"changeAddress\"     (string, optional, default pool address) The AMMOcoin address to receive the change\n"
    ...
    if (request.params.size() > 1) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(changeAddress))
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid AMMOcoin address");
        }
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            "lockunspent unlock transparent [{\"txid\":\"txid\",\"vout\":n},...]\n"
            "\nUpdates list of temporarily unspendable outputs.\n"
            "Temporarily lock (unlock=false) or unlock (unlock=true) specified transparent (transparent=true) or shielded (transparent=false) transaction outputs.\n"
            ...
            "2. transparent       (boolean, required) Whether the given transaction outputs are transparent (true) or shielded (false)\n"
            ...
            HelpExampleCli("lockunspent", "false true \"[{\\\"txid\\\":\\\"a08e6907dbbd3d809776dbfc5d82e371b764ed838b5655e72f463568df1aadf0\\\",\\\"vout\\\":1}]\"") +
            "\nList the locked transactions\n" +
            HelpExampleCli("listlockunspent", "") +
            "\nUnlock the transaction again\n" +
            HelpExampleCli("lockunspent", "true true \"[{\\\"txid\\\":\\\"a08e6907dbbd3d809776dbfc5d82e371b764ed838b5655e72f463568df1aadf0\\\",\\\"vout\\\":1}]\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("lockunspent", "false false, \"[{\\\"txid\\\":\\\"a08e6907dbbd3d809776dbfc5d82e371b764ed838b5655e72f463568df1aadf0\\\",\\\"vout\\\":1}]\""));
    ...
    if (request.params.size() == 2)
        RPCTypeCheck(request.params, {UniValue::VBOOL, UniValue::VBOOL});
    else
        RPCTypeCheck(request.params, {UniValue::VBOOL, UniValue::VBOOL, UniValue::VARR});

    bool fUnlock = request.params[0].get_bool();
    bool transparent = request.params[1].get_bool();

    if (request.params.size() == 2) {
        if (fUnlock) transparent ? pwallet->UnlockAllCoins() : pwallet->UnlockAllNotes();
        return true;
    }

    UniValue output_request = request.params[2].get_array();

    // Create and validate the COutPoints first.
    std::vector<COutPoint> outputs;
    std::vector<SaplingOutPoint> saplingOutputs;
    transparent ? outputs.reserve(output_request.size()) : saplingOutputs.reserve(output_request.size());
    ...
    for (unsigned int idx = 0; idx < output_request.size(); idx++) {
        ...
        if (transparent) {
            ...
            is_locked = pwallet->IsLockedCoin(outpt.hash, outpt.n);
        } else {
            const SaplingOutPoint op(txid, nOutput);
            if (op.n >= wtx.tx->sapData->vShieldedOutput.size()) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, vout index out of bounds");
            }
            if (pwallet->IsSaplingSpent(op)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, expected unspent output");
            }
            is_locked = pwallet->IsLockedNote(op);
        }
        ...
        transparent ? outputs.push_back(COutPoint(txid, nOutput)) : saplingOutputs.push_back(SaplingOutPoint(txid, nOutput));
    }
    ...
    for (const SaplingOutPoint& op : saplingOutputs) {
        if (fUnlock) pwallet->UnlockNote(op);
        else
            pwallet->LockNote(op);
    }
...
UniValue listlockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 0)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"transparent\": [                 (array of json objects)\n"
            "     {\n"
            ...
            "     ...\n"
            "   ],\n"
            "  \"shielded\": [                    (array of json objects)\n"
            "     {\n"
            "       \"txid\": \"transactionid\"     (string) The transaction id locked\n"
            "       \"vShieldedOutput\": n        (numeric) The vout value\n"
            "     },\n"
            "     ...\n"
            "   ],\n"
            "}\n"
    ...
    UniValue ret(UniValue::VOBJ);
    UniValue transparent(UniValue::VARR);
    UniValue shielded(UniValue::VARR);
    ...
    std::set<SaplingOutPoint> sOps = pwallet->ListLockedNotes();
    for (const SaplingOutPoint& op : sOps) {
        UniValue o(UniValue::VOBJ);

        o.pushKV("txid", op.hash.GetHex());
        o.pushKV("vShieldedOutput", (int)op.n);
        shielded.push_back(o);
    }
    ret.pushKV("transparent", transparent);
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in ammocoin.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in ammocoin.conf\n"
...
UniValue setautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.empty() || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same AMMOcoin address\n"
...
{ //  category              name                        actor (function)           okSafe argNames
  //  --------------------- ------------------------    -----------------------    ------ --------
    ...
    { "wallet",             "lockunspent",              &lockunspent,              true,  {"unlock", "transparent", "transactions"} },
