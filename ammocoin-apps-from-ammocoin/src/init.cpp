 ...
 */
const char * const AMMOCOIN_PID_FILENAME = "ammocoin.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", AMMOCOIN_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("ammocoin-shutoff");
...
// Store connection for proper disconnection with newer Boost
static boost::signals2::scoped_connection rpcNotifyBlockConnection;

void OnRPCStarted()
{
    rpcNotifyBlockConnection = uiInterface.NotifyBlockTip.connect(RPCNotifyBlockChange);
}

void OnRPCStopped()
{
    rpcNotifyBlockConnection.disconnect();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", AMMOCOIN_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", AMMOCOIN_PID_FILENAME));
#endif
...
{
    util::ThreadRename("ammocoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that AMMOcoin is running in a usable environment with all
...
{
    // Make sure only a single AMMOcoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", AMMOCOIN_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if AMMOcoin is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if AMMOcoin is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //AMMOcoin specific: zerocoin and spork DB's
                ...
                // AMMOcoin: load previous sessions sporks if we have them.
