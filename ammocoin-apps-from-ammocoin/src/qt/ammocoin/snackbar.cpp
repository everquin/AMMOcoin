...
#include "qt/ammocoin/forms/ui_snackbar.h"
#include "qt/ammocoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(AMMOCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &AMMOCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
